...
do
    ...
    echo "[INFO] docker buildx build --push --platform='$PLATFORMS' --build-arg version=$DOCKER_VERSION_ARG --build-arg KAFKA_VERSION=$KAFKA_VERSION --build-arg SCALA_VERSION=$SCALA_VERSION --tag $REGISTRY/$REGISTRY_ORGANIZATION/$PROJECT_NAME:$CURRENT_TAG $DOCKERFILE_DIR"
done